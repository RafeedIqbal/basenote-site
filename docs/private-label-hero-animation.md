...
- Scroll position remains fractional while moving. Adjacent poses blend with premultiplied alpha, and the SVG anchors interpolate by the same amount, so the final pixels of momentum scrolling do not snap between whole frames. After 250 ms without progress changes, the pose eases to the nearest actual source frame over 140 ms. New scrolling, reduced motion, failures, and route teardown cancel that settle immediately. The canvas stays within the source resolution, and animated updates avoid forced layout reads.
...
Resting-pose checks verified exact source-frame pixels, rounding in both directions, cancellation by new input, and mobile/reduced-motion/route cleanup. The resting desktop canvas matched source frame 095 with zero differing color channels: [settled desktop](../output/playwright/private-label-hero-performance/settled-desktop.png).
